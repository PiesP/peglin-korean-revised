...
확인하는 데 도움이 됩니다. 관리자가 검토 후 번역에 반영하면 새 패치 릴리스가
자동으로 만들어집니다. 직접 번역 파일을 수정해도 되며, 모든 Pull Request는
관리자의 승인 후 수동으로 병합됩니다.
...
제공합니다. 설치용 ZIP에도 MIT 고지와 번역 자료 고지가 들어 있습니다.
`translation/`의 번역 자료와 생성된 오버레이 및 패치 압축 파일에 담긴 번역 자료는
MIT 라이선스의 범위에 포함되지 않습니다. 압축 파일에 포함된 프로젝트 코드와
문서는 MIT 라이선스가 적용됩니다. 기여 문구는 기여 안내에 적힌 허락 범위에서만
사용할 수 있으며, Peglin과 게임 내 콘텐츠의 권리는 각 권리자에게 있습니다. 이
저장소는 게임 콘텐츠에 대한 별도 권리를 부여하지 않습니다. 자세한 범위는
