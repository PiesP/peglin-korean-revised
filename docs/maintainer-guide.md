...
외부 공개 전에 `master` 보호 규칙을 적용하고 다음 항목을 확인합니다.

- PR을 통해서만 병합하고, 승인 1개 이상과 코드 소유자 승인을 요구합니다.
- 새 커밋이 올라오면 기존 승인을 취소하고, 마지막 커밋도 별도로 승인받게 합니다.
- 번역 검증 workflow의 성공을 필수 상태 검사로 설정합니다.
- 관리자 우회, 직접 푸시, 강제 푸시, 브랜치 삭제를 허용하지 않습니다.
- 저장소의 자동 병합을 끈 상태로 두고, 관리자가 승인과 검사를 확인한 뒤 직접
  병합합니다.

`.github/CODEOWNERS`는 전체 변경의 소유자를 `@PiesP`로 지정합니다. 따라서
코드 소유자 승인을 요구하면 저장소 관리자 승인이 필요합니다. 현재 저장소가
비공개 상태에서 해당 보호 규칙을 지원하지 않는 계정 요금제를 사용한다면 규칙을
적용하지 못한 채 공개하지 않습니다. 공개 전환 또는 지원되는 요금제에서 규칙을
적용한 다음 GitHub에서 실제 적용 여부를 확인합니다.

저장소에 연결된 self-hosted runner도 공개 전에 등록 해제합니다. 저장소 workflow는
GitHub 제공 runner만 사용하며, 외부 PR 코드를 self-hosted 환경에서 실행하지
않습니다.
